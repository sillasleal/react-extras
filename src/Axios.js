    ...
    async submit(props) {
        ...
        try {
            ...
            if ([ 'post', 'put', 'patch' ].indexOf(method) > -1) {
                if (config.inRoute) {
                    success = await this.props.instance[method](`${reqUrl}/${config.inRoute}`, data, requestConfigs);
                } else {
                    success = await this.props.instance[method](reqUrl, data, requestConfigs);
                }
            } else {
                if (config.inRoute) {
                    success = await this.props.instance[method](`${reqUrl}/${config.inRoute}`, requestConfigs);
                } else {
